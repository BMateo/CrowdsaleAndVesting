    ...
    UUPSUpgradeable
{
    ...
    function createVestingSchedule(
        ...
        require(
            roles.hasRole(keccak256("ICO_ADDRESS"), msg.sender),
            "Caller is not crowdsale contract nor the admin"
        );
        require(
        ...
        returns (uint256)
    {
        ...
        if (
            (currentTime < startTime.add(vestingSchedule.cliff)) ||
            vestingSchedule.revoked == true ||
            !startInitialized
        ) {
            return 0;
    ...
    function addTotalAmount(uint256 _amount, bytes32 _scheduleId) external {
        ...
        require(
            this.getWithdrawableAmount() >= _amount,
            "TokenVesting: cannot update vesting schedule because not sufficient tokens"
        );
    ...
    function setStartTime(uint256 _timestampStart) external onlyOwner {
        require(
            _timestampStart > block.timestamp,
